...
def get_validation_prompts_and_images(args):
    """Read the dataset info file and return the validation_prompt"""
    try:
        with open(os.path.join(args.train_data_dir, "dataset_info.json"), "r") as f:
            dataset_info = json.loads(f.read())
        validation_prompts = dataset_info.get("validation_prompts")
        validation_images = dataset_info.get("validation_images")
        return validation_prompts, validation_images
    except Exception as e:
        logger.error(
            f"Could not find/read/parse dataset_info.json file, exception: '{e}'. "
            f"To train the model, please include "
            "dataset_info file with json file of the format "
            "{'validation_prompts': [<<validation_prompt>>], 'validation_images': [<<validation_image_path>>]}"
        )
        raise e
...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step):
    ...
    validation_prompts, validation_images = get_validation_prompts_and_images(args)
    if len(validation_image) == len(validation_prompt):
        ...
        validation_images = validation_image
        validation_prompt = validation_prompt * len(validation_image)
    else:
        raise ValueError(
            "number of `validation_image` and `validation_prompt` should be checked in `parse_args`"
        )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.environ.get("WANDB_PROJECT", "sdxl-controlnet"),
        help=(
    ...
    validation_prompts, validation_images = get_validation_prompts_and_images(args)
    if validation_prompts is not None and validation_images is None:
        raise ValueError("`validation_image` must be set if `validation_prompt` is set")

    if validation_prompts is None and validation_images is not None:
        raise ValueError("`validation_prompt` must be set if `validation_image` is set")

    if (
        validation_images is not None
        and validation_prompts is not None
        and len(validation_images) != 1
        and len(validation_prompts) != 1
        and len(validation_images) != len(validation_prompts)
    ):
        raise ValueError(
...
def main(args):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            args.tracker_project_name, init_kwargs={"wandb": {"allow_val_change": True}}
        )
...
def extract_pretrained_model_tarball(
    model_tarball_dir: str, extracted_model_dir: str
) -> None:
    input_model_path = next(Path(model_tarball_dir).glob("*.tar.gz"))
...
if __name__ == "__main__":
    ...
    # if path is relative path of local file, extract the pre-trained model tarball to the /tmp directory
    if args.pretrained_model_name_or_path.startswith("/"):
        extract_pretrained_model_tarball(
