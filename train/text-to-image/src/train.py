...
def get_validation_prompt(args):
    ...
    try:
        ...
        logger.error(
            ...
            "dataset_info file with json file of the format {'validation_prompt': <<validation_prompt>>}"
        )
        raise e
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.environ.get("WANDB_PROJECT", "sdxl-text2image-lora"),
        help=(
...
def main(args):
    ...
    def load_model_hook(models, input_dir):
        ...
        if args.train_text_encoder:
            logger.info("train text encoder")
            _set_state_dict_into_text_encoder(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if args.checkpoints_total_limit is not None:
                            checkpoints = os.listdir("/opt/ml/checkpoints")
                            checkpoints = [
                            ...
                            if len(checkpoints) >= args.checkpoints_total_limit:
                                ...
                                for removing_checkpoint in removing_checkpoints:
                                    removing_checkpoint = os.path.join(
                                        "/opt/ml/checkpoints", removing_checkpoint
                                    )
                                    shutil.rmtree(removing_checkpoint)

                        save_path = os.path.join(
                            "/opt/ml/checkpoints", f"checkpoint-{global_step}"
                        )
        ...
        if accelerator.is_main_process:
            validation_prompt = get_validation_prompt(args)

            if validation_prompt and epoch % args.validation_epochs == 0:
                logger.info(
                        ...
                        tracker.log(
                            {
                                "validation": [
                                    wandb.Image(
                                        image, caption=f"{i}: {validation_prompt}"
                                    )
